...
def test_app():
    ...
    :yield: Flask applicationwith a context, ready for testing
    '''
...
def test_full_random_game(test_app):
    ...
    if language == 'bad':
        assert response.status_code == 400, 'Wrong response for invalid lnaguage'
        return
    ...
    assert ( ((game_data['result'] ==  'lost') and (game_data['bad_guesses'] == 6)) or
            ((game_data['result'] == 'won') and (game_data['bad_guesses'] < 6)) )
    ...
    assert response.status_code == 403, 'Post-games guess should be forbidden'
