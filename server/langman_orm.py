...
class User(base_games):
    '''Tables ``users`` with fields:
        * ``user_id`` - UUID primary key of lenght 38
        * ``user_name`` - String of maximum length 30
        * ``num_games`` - Integer coung of games started
    ...
    def _decr_json_field(self, field, key):
        ...
        ``field`` is a JSON text strin. (Does not commit)'''
        d = json.loads(getattr(self, field))
        d[key] = d.get(key, 0) - 1
        setattr(self, field, json.dumps(d))
...
class Game(base_games):
    ...
    reveal_word = Column(types.String(length=30), nullable=False)
