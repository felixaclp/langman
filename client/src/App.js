...
import { SignInScreen, PlayScreen, WinScreen, LoseScreen } from './screen';
...
class App extends Component{
            ...
            if (this.state.loginToken) {
                axios({
                    ...
                    headers: {
                        ...
                        //console("Token " + this.state.loginToken)    
                        axios({
                            method: 'get',
                ...
                if (error.response.data.msg === 'User claims verification failed') {
                    if (numCalls === 4) {
                        ...
                        axios({
                            ...
                            //with successful login, update the loging token
                            ...
                            axios({
                                ...
                                // on success, udpate the access token
                                this.setState({ 
                                    gameToken: response.data.access_token
                                });
                            });
                        }).finally( () =>{
                            // and now try again, with a possible corrected token
    ...
    render() {
        ...
        console.log('the current state is: ' + gameStatus)
        let screen = <></>;
        if (gameStatus === 'logged out') {
            ...
            return <div>Unexpected status: {gameStatus}</div>;
        }
